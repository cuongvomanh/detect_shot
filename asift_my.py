...
if __name__ == '__main__':
    ...
    def match_and_draw(win, pre_img, next_img):
        ...
        explore_match(win, pre_img, next_img, kp_pairs, None, H)
    ...
    while True:
        ...
        kp1, desc1 = affine_detect(detector, pre_img, pool=pool)
        kp2, desc2 = affine_detect(detector, next_img, pool=pool)
        print('img1 - %d features, img2 - %d features' % (len(kp1), len(kp2)))

        match_and_draw('affine find_obj', pre_img, next_img)
        ch = cv.waitKey(5)
        if ch == 27:
            break
